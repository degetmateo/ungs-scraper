    ...
    createSubject(data) {
        try {
            ...
            const correlatives = data.correlatives.trim().length > 0 ? data.correlatives
                ...
                .split('\n').join('-')
                .split('\n').map(s => s.trim()) : [];
            return new subject_1.default(name, course_regime, weekly_hours, total_hours, data.correlatives.trim(), correlatives);
        }
        catch (error) {
            throw error;
        }
        ...
        for (const subject of subjects) {
            ...
            for (const s2 of subjects) {
                if (normalizedName === this.normalize(s2.getName()))
                    continue;
                const correlatives = s2.getParsedCorrelatives();
